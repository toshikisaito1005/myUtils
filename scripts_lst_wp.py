...
class ToolsLSTSim():
    ...
    def plot_config(self):
        ...
        u_alma, v_alma = self._get_baselines(this_data,this_data,decl=0,tinteg=1)
        u_lst_center, v_lst_center = self._get_baselines(np.array([0,0]),this_data,decl=0,tinteg=1)
    ...
    def _get_baselines(self,x,y,decl=60,tinteg=0):
        ...
        for this_t in trange:
            D = list_dist
            L = -67.755 # degree
            A = np.radians(list_angle)
            dec = np.radians(decl) + np.radians(this_t)

            X = list_dist*(-np.sin(L)*np.cos(A))
            Y = D*np.sin(A)

            this_u = np.sin(L)*X + np.cos(L)*Y
            this_v = -X*np.sin(dec)*np.cos(L) + Y*np.sin(dec)*np.sin(L)
