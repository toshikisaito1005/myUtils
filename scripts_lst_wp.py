...
class ToolsLSTSim():
    ...
    def plot_config(self):
        ...
        u_alma, v_alma = self._get_baselines(this_data,this_data,decl=45,tinteg=12)
        u_lst_center, v_lst_center = self._get_baselines(np.array([0,0]),this_data,decl=45,tinteg=12)
    ...
    def _get_baselines(self,x,y,decl=60,tinteg=0):
        ...
        for this_t in trange:
            ...
            this_v = X*np.sin(dec)*np.cos(H_h) - Y*np.cos(dec)
