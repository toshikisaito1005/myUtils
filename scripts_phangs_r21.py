...
class ToolsR21():
    ...
    def plot_model_std(
        ...
        # set plt, ax
        ad = [0.03,0.97,0.20,0.97]
        plt.figure(figsize=(20,7))
        plt.subplots_adjust(bottom=0.10, left=0.20, right=0.97, top=0.97)
        gs = gridspec.GridSpec(nrows=3, ncols=3)
        ax1 = plt.subplot(gs[0:3,0:1])
        ax2 = plt.subplot(gs[0:3,1:2])
        ax3 = plt.subplot(gs[0:3,2:3])
        myax_set(ax1, "both", None, None, None, label1, label3, adjust=ad)
        myax_set(ax2, "both", None, None, None, label1, label4, adjust=ad)
        myax_set(ax3, "both", None, None, None, label2, label4, adjust=ad)
