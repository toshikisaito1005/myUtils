...
class ToolsCIGMC():
    ...
    def plot_larson(
        ...
        fig = plt.figure(figsize=(10,10))
        gs  = gridspec.GridSpec(nrows=200, ncols=200)
        ax1 = plt.subplot(gs[40:200,10:170])
        ax2 = plt.subplot(gs[0:30,10:170])
        ax3 = plt.subplot(gs[40:200,170:200])
        ad  = [0.215,0.83,0.10,0.90]
        myax_set(ax1, None, xlim, ylim, None, xlabel, ylabel, adjust=ad)
        myax_set(ax2, None, xlim, None, None, None, None)
        myax_set(ax3, None, None, ylim, None, None, None)
    ...
    def _scatter_hist(self, x, y, ax, ax_histx, ax_histy, color, xlim, ylim, marker="o", offset=0):
        ...
        this_txt = str(int(10**np.percentile(x,16)))+"-"+str(int(10**np.percentile(x,50)))+"-"+str(int(10**np.percentile(x,84)))+" ($M_{\odot}$ pc$^{-2}$)"
